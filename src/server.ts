...
<h1>KiraAI Route</h1>
<div class="subtitle">
Connect Kira AI to OpenAI-compatible coding tools
</div>
</div>
...
id="apiKey"
type="password"
placeholder="Paste your Kira API key"
autocomplete="off"
>
<div class="hint">
Your API key is kept in the local KiraAI Route process.
</div>
</div>
...
<div class="hint">
Codex will connect through your local KiraAI Route gateway.
</div>
</div>
...
async function loadModels() {
  ...
  const freeModels = models.filter(
    model => !model.balance_required
  );

  const balanceModels = models.filter(
    model => model.balance_required
  );
  ...
  for (const model of balanceModels) {
    ...
    option.value = model.id;
    option.textContent = model.name;
    balanceGroup.appendChild(option);
  }
...
startButton.addEventListener(
  ...
  async () => {
    ...
    if (!apiKey) {
      status(
        "Please enter your Kira API key.",
        "error"
      );
      return;
    }
    ...
    try {
      ...
      status(
        "✓ Kira API connected successfully.",
        "success"
      );
  ...
  try {
    ...
    if (!body?.apiKey?.trim()) {
      return reply.code(400).send({
        error: {
          message: "Kira API key is required."
        }
      });
    }
  ...
  try {
    if (!hasKiraApiKey()) {
      return reply.code(400).send({
        error: {
          message:
            "Kira API key is not configured."
        }
      });
    }

    const result =
      await testKiraConnection();
